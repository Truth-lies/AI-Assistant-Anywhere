...
- `src/store/messagePipeline.ts`：消息编排辅助函数（系统提示拼装、多模态文本提取、图片追问识别）
...
- **✨ 更优雅的聊天 UI**：主聊天页升级卡片化空状态、轻量悬浮层级与更精致的输入区胶囊布局，视觉更统一、可读性更好。
- **🧠 多层 RAG 记忆**：感性层（情感分析）+ 理性层（用户画像）+ 历史层（对话记忆）+ 通用知识库。
- **🧾 RAG 检索可解释**：注入上下文时附带命中原因、来源标记与相似度分数，便于追踪回答依据。
